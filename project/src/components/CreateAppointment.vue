<template>
  ...
  <div class="container mt-5">
    <div class="row justify-content-center">
      <div class="col-md-6">
        <form @submit.prevent="createAppointment" class="shadow p-4">
          <h4 class="mb-4 text-center">Create Appointment</h4>
          <!-- Form fields for appointment data -->
          <div class="mb-3">
            <select class="form-select" v-model="appointment.doctor_id">
              <option value="" disabled>Select Doctor</option>
              <option v-for="doctor in doctors" :key="doctor.id" :value="doctor.id">{{ doctor.user.name }} - {{
                doctor.specialization.specialization_title }}</option>
            </select>
          </div>
          <div class="mb-3">
            <input type="date" class="form-control" v-model="appointment.appointment_date"
              placeholder="Enter Appointment Date">
          </div>
          <!-- Include other appointment fields here -->
...
export default {
  ...
  created() {
    // Fetch doctors data when the component is created
    this.fetchDoctors();
  ...
  methods: {
    fetchDoctors() {
      ...
      axios.get(`${BASE_URL}/patient/doctors`)
        .then(response => {
    ...
    createAppointment() {
      ...
      axios.post(`${BASE_URL}/patient/createAppointments`, this.appointment)
        .then(() => {
          this.clearForm();
          Swal.fire({
            icon: 'success',
            title: 'Appointment created successfully!',
            text: 'You have successfully created a appointment.',
            confirmButtonText: 'OK'
          }).then(() => {
            this.$router.push({ name: 'appointment-list-patients' });
          });
        })
        .catch(error => {
          Swal.fire({
    ...
    clearForm() {
      this.appointment.patient_id = this.appointment.doctor_id = this.appointment.appointment_date = "";
      this.doctors = {};
    },
  }
};
